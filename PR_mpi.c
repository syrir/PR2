...
int main(int argc, char **argv)
{
	...
	if(rank==0)
	{
		...
		show_mat(array,size_of_matrix);
		for(int i=0;i<size_of_matrix-size_of_submatrix+1;i++)
		{	
			printf("wysylam i= %d \n",i);
			for(int j=0+i;j<size_of_submatrix;j++)
			{
				for(int k=0;k<size_of_matrix;k++)
				{	
					temp[j-i][k]=array[j][k];
				}
			}
			printf("rozmiar: %d adres wysylki: %d \n",size,(i%size)+1);
			MPI_Send(&(temp[0][0]), size_of_matrix*size_of_submatrix, MPI_INT, (i%(size-1))+1, DATA, MPI_COMM_WORLD);
		}
	    	while(finished!=size-1)
		{
			...
			switch (status.MPI_TAG)
			{
				case WYNIK:
					...
					show_mat(master,size_of_submatrix);
					printf("\n");
					break;
				case END:
	...
	else
	{
		while(a)
		{	
			MPI_Recv(&(local[0][0]), size_of_matrix*size_of_submatrix, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			switch (status.MPI_TAG)
			{
				case DATA:
